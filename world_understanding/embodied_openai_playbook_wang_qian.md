...
- 选 1 个你最想下注的具身路线/公司，把它写成 3 条**可证伪**假设（“什么条件下端到端赢/分层赢”）。
- 立刻做一个“最小回归集”：固定 3–5 个任务 + 失败分类字段（否则你会被 demo 误导）。
- 用 `worksheets/decision_one_pager.md` 写清：退出条件（停机点）与重启条件（证据门槛）。
...
- 本篇关键槽位：对象、场景、最小回归集（任务+失败分类）、ROI 口径、退出/重启阈值。

---

## 关联猜想（meta）

- `H-0004`：具身是否出现 scaling，关键往往是“信号密度 + 回归体系 + 数据闭环”，而不是口号式路线争论。
