...
|---|---|---|---|---|---|---|
| 训练侧对齐（把安全目标塞进微调） | Phase2/3（门禁化/治理）但成本过高 | 安全与能力能否同时回归；多版本维护成本是否线性上升 | 单一数据集/单一攻击集好看数字 | 链路复杂化→调参盲盒→版本爆炸 | 证明“部署期兜底”能把治理成本从训练侧迁走 | 组织不愿维护多条安全训练分支 |
| 事后修复（再训一轮/适配训练） | Phase0→1（能修但难工具链化） | 是否真的独立于微调成本函数；对 unseen/adaptive 是否稳 | “无需训练”但暗含额外训练/信号 | 仍回到训练成本；上线节奏被拖慢 | 把修复做成固定成本、可流水线化 | 依赖训练信号/二次训练，成本随版本爆 |
| 对齐感知量化（同方向工作） | Phase0（主张成立）→ Phase1（工具链化） | unseen/adaptive 增益；误拒率可控；低比特稳定性 | 只报 harmful score 不报误伤/自适应 | Goodhart；攻击适配后收益消失 | **把 Q-realign 推到 Phase1**：可复现实现 + 标准化报告 | 没有代码/复现难；门禁指标不全（误拒/自适应） |

  - **本项目当前定位**：Q-realign 现在更像 **Phase0（论文主张成立）**；要进入可规模化的工程采用，下一步必须推进到 **Phase1（工具链化）**（并把误拒率与自适应攻击写进发布门禁）。
- 当时大多数人忽略/误解什么？
