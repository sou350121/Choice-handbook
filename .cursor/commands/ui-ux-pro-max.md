...
> 本仓库已内置数据与脚本：`.shared/ui-ux-pro-max/`（无需额外安装）。本命令用于把“审美/风格/规范”变成可执行的实现方案。
...
```powershell
python .shared\ui-ux-pro-max\scripts\search.py "latex editor" --domain ux -n 5
```
...
在实现前，先输出一份 1 页以内的 UI spec：
- **Style**：从 `style` 域选 1 个主风格（比如 Minimal / Dark SaaS / Glass）
- **Typography**：从 `typography` 选 1 组字体组合（包含导入与字号层级）
- **Color**：从 `color` 选 1 套 palette（主色/强调/背景/边框/文字）
- **UX**：从 `ux` 选 5-10 条 guideline（只选跟当前页面相关的）
- **Apply plan**：列出将修改的组件/文件 + 风险点（不改业务逻辑优先）
