...
- 给训练加一个“稳定性指标板”：逐层 Amax + layer0 canary + 跨 seed 方差，并写明停机阈值。  
- 直接用 `AGENT.md` 的 `Experiment Prompt` 把压力测试流程写成可执行回归清单。
...
本篇的 Plan Prompt 已上移到 `AGENT.md`（避免在每篇里重复模板）；你只要把“模型类型/连接结构/规模/阈值”填进去即可。

---

## 关联猜想（meta）

- `H-0001`：可规模化训练更需要“可保证的稳定性约束”与逐层监控（本篇提供机制与信号）。
